...
const ProductBestSale = () => {
  ...
  const [ProductBestSale, setProductBestSale] = useState([]);
  ...
  useEffect(() => {
    const fetchData = async () => {
      const result = await ProductService.product_bestseller(4);
      setProductBestSale(result.products);
    };
    fetchData();
  }, []);
