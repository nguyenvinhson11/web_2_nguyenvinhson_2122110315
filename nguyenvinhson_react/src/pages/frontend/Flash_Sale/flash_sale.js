...
const Flashsale = () => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        const filtered = response.slice(0, 4);
        console.log('data nhận được:', filtered);
  ...
  return (
    <section className="bg-flash-sell py-3">
      <div className="container max-w-7xl mx-auto">
        <div className="grid grid-cols-12">
          <div className="col-span-8">
            <h1 className="text-white text-4xl text-center w-72 px-2 py-5 rounded-md bg-category font-bold uppercase mt-5">
              FLASH SALE
            </h1>
          </div>
          <div className="col-span-4">
            <div className="bg-white rounded-md shadow px-5 py-4">
              <h4 className="text-black mb-3 font-bold text-lg">
                THỜI GIAN ÁP DỤNG
              </h4>
              <div className="flex justify-center gap-3">
                {Object.entries(timeLeft).map(([label, value]) => (
                  <div key={label} className="text-center">
                    <div className="text-xl font-bold border-2 border-red-600 px-4 py-1 rounded">
                      {value}
                    </div>
                    <div className="text-xs uppercase mt-1 text-gray-600">
                      {label}
                    </div>
                  </div>
                ))}
